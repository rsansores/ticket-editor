...
fn unresolved_path_renders_empty_by_default_and_fake_in_placeholder_mode() {
    ...
    let print =
        render_png_with_options(&doc, &empty_data, fonts, &RenderOptions::default()).unwrap();
    let editor =
...
fn unresolved_paths_reports_typos_and_respects_scopes() {
    let doc: TicketDoc = serde_json::from_value(json!({
        ...
        "regions": [
            { "id": "loop", "start_row": 2, "end_row": 3, "source": "items" },
        ...
        "elements": [
            ...
            { "id": "vol",  "row": 2, "col": 0, "type": "variable", "path": "qty", "length": 8 },
            { "id": "oops", "row": 2, "col": 10, "type": "variable", "path": "qtty", "length": 8 },
            { "id": "n",    "row": 2, "col": 20, "type": "variable", "path": "row.number", "length": 3 },
            { "id": "bad",  "row": 2, "col": 24, "type": "variable", "path": "row.amount", "length": 8 },
    ...
    // A declared row-computed makes row.amount valid inside its band.
    ...
    assert!(!missing.contains(&"row.amount".to_string()));
}
...
fn image_from_variable_resolves_and_missing_draws_nothing_on_print() {
    ...
    let missing_editor = render_png_with_options(
        &doc,
        &serde_json::Value::Null,
        fonts,
        &RenderOptions::placeholders(),
    )
    .unwrap();
...
fn row_scoped_qr_never_fakes_and_max_lines_zero_is_unbounded() {
    ...
    let editor =
        render_png_with_options(&qr_doc, &json!({}), fonts, &RenderOptions::placeholders())
            .unwrap();
    let empty =
        render_png_with_options(&blank, &json!({}), fonts, &RenderOptions::placeholders()).unwrap();
...
/// A retail sale: a line-items loop with QTY, PRICE and a row-computed
/// AMOUNT column, plus a doc-level total below the band.
fn row_computed_doc() -> TicketDoc {
    serde_json::from_value(json!({
        "version": 2, "paper": { "width_chars": 40 },
        "computed": [{ "name": "total", "formula": "sum(items, qty * price)" }],
        "regions": [{
            "id": "rows", "start_row": 1, "end_row": 2, "source": "items",
            "computed": [
                { "name": "amount", "formula": "round(qty * price, 2)" },
                // References an earlier row-computed of the same band.
                { "name": "amount_tax", "formula": "round(row.amount * 1.16, 2)" }
            ]
        }],
        "elements": [
            { "id": "hdr", "row": 0, "col": 0, "type": "text", "content": "QTY   PRICE   AMOUNT" },
            { "id": "n",   "row": 1, "col": 0, "type": "variable", "path": "row.number", "length": 2 },
            { "id": "v",   "row": 1, "col": 3, "type": "variable", "path": "qty", "length": 6,
              "number": { "decimals": 2, "rounding": "half_up", "thousands": false } },
            { "id": "p",   "row": 1, "col": 10, "type": "variable", "path": "price", "length": 7,
              "number": { "decimals": 2, "rounding": "half_up", "thousands": false } },
            { "id": "amt", "row": 1, "col": 18, "type": "variable", "path": "row.amount", "length": 10,
              "align": "right",
              "number": { "decimals": 2, "rounding": "half_up", "thousands": true } },
            { "id": "iva", "row": 1, "col": 29, "type": "variable", "path": "row.amount_tax", "length": 10,
              "align": "right" },
...
fn row_computed_single_item_has_correct_value() {
    // One item: row.amount = 5 * 20 = 100. The render must equal a doc where
    ...
    let by_formula = render_png(&doc, &json!({ "items": [ { "qty": 5, "price": 20 } ] })).unwrap();
    // Same doc, but the amount comes precomputed in the data and the element binds
    // to it directly.
    let mut direct: TicketDoc = row_computed_doc();
    direct.regions[0].computed = vec![serde_json::from_value::<ticket_core::Computed>(
        json!({ "name": "amount_tax", "formula": "round(amount * 1.16, 2)" }),
    )
    .unwrap()];
    // rebind row.amount -> amount (denormalized into the item)
    let doc_json = serde_json::to_string(&direct)
        .unwrap()
        .replace("row.amount\"", "amount\"");
    let direct: TicketDoc = serde_json::from_str(&doc_json).unwrap();
    let denormalized = render_png(
        &direct,
        &json!({ "items": [ { "qty": 5, "price": 20, "amount": 100 } ] }),
    )
    .unwrap();
    assert_eq!(
        by_formula, denormalized,
        "row.amount must render like real item data"
    );
}
...
fn row_formula_mixes_calc_and_root_paths() {
    ...
    let a = render_png(
        &doc,
        &json!({ "shipping": { "flat": 5 }, "items": [ { "amount": 100 } ] }),
    )
    .unwrap();
    let b = render_png(
        &doc,
        &json!({ "shipping": { "flat": 9 }, "items": [ { "amount": 100 } ] }),
    )
    .unwrap();
...
fn row_paths_outside_a_band_render_empty_never_fake() {
    ...
    let doc: TicketDoc = serde_json::from_value(json!({
        "version": 2, "paper": { "width_chars": 24 },
        "elements": [
            { "id": "x", "row": 0, "col": 0, "type": "variable", "path": "row.amount", "length": 10 }
        ]
    }))
    .unwrap();
    let blank: TicketDoc = serde_json::from_value(json!({
        "version": 2, "paper": { "width_chars": 24 }, "elements": [] }))
    .unwrap();
    let fonts = &Fonts::builtin().unwrap();
...
fn conditional_band_evaluates_declared_row_values_once() {
    ...
    assert_ne!(
        a, b,
        "the declared row value must be live on a conditional band"
    );
}
...
fn wrapped_lines_push_content_below_down() {
    // The field-wrap bug: a long customer name in a wrapped `length: 39`
    // field at row 0, ">> INVOICE REQUESTED <<" at row 1. With the fix the
    ...
    let expected: TicketDoc = serde_json::from_value(json!({
        ...
        "elements": [
            { "id": "l1", "row": 0, "col": 0, "type": "text",
              "content": "Extraordinary Pet Supplies and Grooming" },
            { "id": "l2", "row": 1, "col": 0, "type": "text", "content": "LLC" },
            { "id": "m", "row": 2, "col": 0, "type": "text",
              "content": ">> INVOICE REQUESTED <<" }
        ]
    }))
    .unwrap();
    let data = json!({ "customer": "Extraordinary Pet Supplies and Grooming LLC" });
    assert_eq!(
...
fn markers_report_post_flow_rows_and_leave_no_ink() {
    ...
    let hits: Vec<(&str, u32)> = out
        .markers
        .iter()
        .map(|m| (m.name.as_str(), m.row))
        .collect();
    ...
    bare.elements
        .retain(|e| !matches!(e.kind, ticket_core::ElementKind::Marker { .. }));
    let bare_png = render(&bare, &data, fonts, &RenderOptions::default())
        .unwrap()
        .png;
