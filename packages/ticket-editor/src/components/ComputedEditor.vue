  ...
  <div class="te-modal-backdrop" @click.self="emit('cancel')">
    <div class="te-modal" role="dialog" aria-modal="true">
      <header class="te-modal-head">
        <strong>{{ modelValue && modelValue.name ? t('calcEditTitle') : t('calcNewTitle') }}</strong>
        <button class="te-modal-x" type="button" :aria-label="t('cancel')" @click="emit('cancel')">✕</button>
      </header>
      <p class="te-modal-lead">{{ t('calcLead') }}</p>

      <label class="te-field">
        <span>{{ t('calcName') }}</span>
        <input class="te-input" :value="name" :placeholder="t('calcNamePlaceholder')"
      ...
      <div class="te-field">
        ...
        <textarea ref="formulaEl" class="te-input te-formula" rows="3" spellcheck="false"
          :value="formula" :placeholder="t('calcFormulaPlaceholder')"
          @input="formula = ($event.target as HTMLTextAreaElement).value"></textarea>
        <div class="te-inserters">
          <select class="te-mini" :value="varPick" @change="onPickVar">
            ...
            <optgroup v-for="g in varGroups" :key="g.label" :label="g.label">
              <option v-for="(o, i) in g.options" :key="g.label + i" :value="o.insert">{{ o.label }}</option>
            </optgroup>
          </select>
      ...
      <div class="te-preview-line" :class="{ 'has-error': formulaError }">
        ...
        <code v-else class="te-preview-val">{{ result && result.value !== '' ? result.value : t('calcPreviewEmpty') }}</code>
      </div>

      <footer class="te-modal-foot">
        <button class="te-btn te-btn-ghost" type="button" @click="emit('cancel')">{{ t('cancel') }}</button>
        <button class="te-btn te-btn-primary" type="button" :disabled="!canSave" @click="save">{{ t('calcSave') }}</button>
      </footer>
    </div>
  </div>
</template>
...
.te-modal {
  ...
  background: var(--te-card); color: inherit; border: 1px solid var(--te-border);
  border-radius: var(--te-radius); padding: 1rem; display: flex; flex-direction: column; gap: 0.7rem; font-size: 0.85rem;
}
.te-modal-head { display: flex; align-items: center; justify-content: space-between; }
.te-modal-x { border: 0; background: transparent; color: var(--te-muted-fg); cursor: pointer; font-size: 0.9rem; }
.te-modal-lead { margin: 0; color: var(--te-muted-fg); font-size: 0.78rem; }
...
.te-formula { font-family: ui-monospace, monospace; font-size: 0.82rem; resize: vertical; line-height: 1.4; }
.te-err { color: #dc2626; font-size: 0.72rem; }
.te-inserters { display: flex; gap: 0.4rem; }
.te-mini {
  flex: 1; min-width: 0; padding: 0.25rem 0.4rem; border: 1px solid var(--te-input);
  border-radius: calc(var(--te-radius) - 2px); background: var(--te-card); color: var(--te-primary); font: inherit; font-size: 0.76rem; cursor: pointer;
}
.te-fn-doc { margin: 0; font-size: 0.75rem; color: var(--te-muted-fg); }
.te-fn-doc code { font-family: ui-monospace, monospace; color: inherit; }
.te-hint { margin: 0; font-size: 0.72rem; color: var(--te-muted-fg); line-height: 1.35; }
.te-preview-line {
  display: flex; align-items: baseline; gap: 0.5rem; padding: 0.5rem 0.6rem;
  background: var(--te-accent); border-radius: calc(var(--te-radius) - 2px); flex-wrap: wrap;
}
.te-preview-line.has-error { background: color-mix(in srgb, #dc2626 12%, transparent); }
.te-preview-label { color: var(--te-muted-fg); font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.04em; }
.te-preview-val { font-family: ui-monospace, monospace; font-size: 0.8rem; word-break: break-all; }
.te-preview-val.err { color: #dc2626; }
.te-modal-foot { display: flex; justify-content: flex-end; gap: 0.5rem; margin-top: 0.2rem; }
.te-btn { padding: 0.4rem 0.8rem; border-radius: calc(var(--te-radius) - 2px); border: 1px solid transparent; font: inherit; font-size: 0.82rem; cursor: pointer; }
...
.te-btn-primary:disabled { opacity: 0.55; cursor: default; }
</style>
