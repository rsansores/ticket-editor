...
// NOTE ON WASM LOADING: the library build inlines the wasm as a `data:` URL (Vite
// lib mode does this and won't be talked out of it). We *want* that here — the
// renderer decodes the inlined bytes and instantiates them directly
// (see `useRenderer.ts`), instead of relying on the host app's bundler to
// resolve/fetch a wasm URL. That indirection (whether a data: URL or a file URL)
// is exactly what breaks in a consuming Vite app.
